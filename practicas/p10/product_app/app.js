...
function agregarProducto(e) {
    ...
    client.onreadystatechange = function () {
        ...
        if (client.readyState == 4 && client.status == 200) {
            ...
            // Dependiendo del status, se muestra un alert con el mensaje
...
function getXMLHttpRequest() {
    try {
        return new XMLHttpRequest(); // Objeto XMLHttpRequest para navegadores modernos
    } catch (error) {
        console.error("No se pudo crear el objeto XMLHttpRequest:", error);
        return null; // Retorna null si hay un error (muy raro en navegadores modernos)
    }
}
...
function validarJSON(finalJSON){
    ...
    const marcasValidas = ["Lego", "Playmobil"];
